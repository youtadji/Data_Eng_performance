...
public class IntRefactoredMainApproxRangeTree {
    ...
    public static void main(String[] args) {
        System.out.println("=== EXP INT: NoFilter vs Bloom vs ExactRanges vs BulkApprox ===");
    ...
    private static void runExperiment_Int(String suffix) {
        ...
        String outName = "experiment_bulk_approxranges_WITHHUSH_3M_clean_" + suffix + ".csv";
        ...
        try (PrintWriter w = new PrintWriter(outName)) {
            ...
            w.println(
                    "build_size,probe_size,filter_size_bits,clusters,cluster_len,gap_len,target_ranges,"
                            ...
                            + "build_ms_approxranges,probe_ms_approxranges,"
                            + "speedup_probe_bloom,speedup_probe_exactranges,speedup_probe_approxranges,"
                            + "hash_lookups_bloom,bloom_passes,bloom_rejects,"
                            + "hash_lookups_range,range_passes,range_rejects,range_count,range_bytes,"
                            + "hash_lookups_approx,approx_passes,approx_rejects,exact_range_count,approx_range_count,approx_range_bytes,"
                            + "approx_lookup_ratio"
            );
            ...
            for (int buildSize : buildSizes) {
                for (int gapLen : gapLens) {
                    for (int clusters : clusterCounts) {
                        ...
                        // ============================================================
                        // Generate dataset once per dataset configuration
                        // ============================================================
                        ...
                        // ============================================================
                        // Run baselines once per dataset
                        // ============================================================
                        ...
                        // ============================================================
                        // Run bulk approx per target range count
                        // ============================================================
                        for (int targetRangeCount : targetRangeCounts) {
                            ...
                            for (int m : bloomSizes) {
                                ...
                                double approxLookupRatio =
                                        rt.hashLookups == 0 ? 0.0 : ((double) ar.hashLookups / rt.hashLookups);
                                ...
                                w.println(
                                        buildSize + "," + probeSize + "," + m + ","
                                                ...
                                                + fmt(ar.buildMs) + "," + fmt(ar.probeMs) + ","
                                                + fmt3(speedBloomProbe) + "," + fmt3(speedRangeProbe) + "," + fmt3(speedApproxProbe) + ","
                                                + bl.hashLookups + "," + bl.passes + "," + bl.rejects + ","
                                                + rt.hashLookups + "," + rt.passes + "," + rt.rejects + "," + rt.rangeCount + "," + rt.rangeBytes + ","
                                                + ar.hashLookups + "," + ar.passes + "," + ar.rejects + ","
                                                + ar.exactRangeCount + "," + ar.approxRangeCount + "," + ar.rangeBytes + ","
                                                + fmt3(approxLookupRatio)
                                );
                            }
                        }
                    }
